...
4. **Design component spec**:
   ...
   - Walk the metadata node tree, catalog nodes and nodeIds. Pay attention to:
     - **instance nodes**: strong signal for component boundaries
     - **semantic node names**: help understand intent, but do not copy as component names
     - **auto-generated frame names** (e.g., `Frame 2147239210`): no naming value, but node structure still matters
     - **text node names**: contain actual text, help understand what a section is about
   - Based on the above, design a component tree split into three levels:
   ...
   - Naming: PascalCase, reflect what it is or does (e.g., `ChatHeader`, not `Frame27`)
