...
5. **质量和测试**
   ...
   实现需求后，必须立即分析应该如何完成高质量的测试，使用 `test-generator agent` 生成、执行测试用例。

   ```text
   【测试】
   - 相关代码缺少 E2E 测试，正在生成...
   - 相关代码缺少 Performance 测试，正在生成...
   ```
...
1. 你必须主动分析用户意图，优先考虑使用 MCP、Skill 等标准工具来高效完成任务。
2. 如果用户未安装所需 MCP ，你必须使用 `mcp-manager agent` 自动安装或卸载 MCP。

## 工具使用

### 文档、代码检索工具

1. **查看官方文档**: 可使用 `Context7` MCP 来检索文档。
2. **搜索真实代码**: 可使用 `Grep` MCP 来搜索 GitHub 代码。

### 文档编写工具

1. **编写需求、设计文档**: 可使用 `Spec Workflow` MCP 来完成。
